...
- **Level of subjectivity**.  This is variability in terms of relevant importance attributed to each issue given a set of issues. For instance, it is possible that a great many individuals or political leaders would attach a higher importance to individual liberties to secuirty or otherway around. But the question might be rather to understand on what scale one attaches more importance to an issue more than others. So when the objective of the scoring is not simply to order of importance, then a comparative importance with respect to overall observations needs to be tackled.
